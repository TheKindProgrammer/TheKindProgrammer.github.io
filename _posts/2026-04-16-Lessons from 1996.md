...
AI may be real enough that the better trade is not trying to short the most overpriced stocks, but looking downstream at the bottlenecks. If Leopold is right that AI has become an industrial process - giant clusters, power plants, fabs, and eventually gigawatt-scle infrastructure - then the second-order plays in power, cooling, optics, grid equpiment, and memory deserve attention. Amodei's warning is on the other side of that trade: capability can compound faster than the economy can absorb it, and the capex math becomes dangerous if companies buy compute for a demand curve that arrives even a year late. The lesson from 1996 is that the theme can be right, the infrastructure cycle can be real, and the valuation can still leave little room for error.

*However there's been nothing quite like the railroad buildout (so far):*
...
![yale](/assets/post1/yalecomposition.png)
